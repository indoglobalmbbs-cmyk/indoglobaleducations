...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const { form, recaptchaToken } = parseBody(req.body);
    ...
    const { error: insertError } = await supabase.from('indoglobal').insert([
      {
    ...
    res
      .status(500)
      .json({ error: 'Unable to submit your enquiry. Please try again.' });
  }
}
